...
import { CALL_OUTCOMES, type CallOutcome, type OrderStatus } from "@/lib/data/schemas";
import { formatDate, formatPrice, fulfillmentLabel, t } from "@/lib/i18n";
import { canEdit, dzPhoneIntl, NEXT_STATUS } from "@/lib/order";
import { logCall, saveNote, setStatus } from "../actions";
...
export default async function OrderPage({ params }: PageProps<"/admin/orders/[id]">) {
  ...
  const calls = history.filter((h) => h.action === "call").length;
  const wa =
    order.status === "ready"
      ? order.fulfillment === "pickup"
        ? "readyPickup"
        : "readyDelivery"
      : order.status === "new" && calls
        ? "noAnswer"
        : order.status;
  ...
  const editLink = canEdit(order.status) && (
    <Link href={`/admin/orders/${order.id}/edit`} className="inline-flex min-h-11 items-center text-primary">
      {t("admin.orders.edit")}
    </Link>
  );
  type Diff = {
    from?: OrderStatus; to?: OrderStatus; note?: string; outcome?: CallOutcome;
    lines?: [string, number, number][]; fields?: ("name" | "phone" | "wilaya" | "address" | "fulfillment")[]; total?: [number, number];
  };
  const editSummary = (d: Diff) =>
    [
      ...(d.lines ?? []).map(([l, a, b]) => `${l} : ${a} → ${b}`),
      ...(d.fields ?? []).map((f) => t(`checkout.${f}`)),
      ...(d.total && d.total[0] !== d.total[1] ? [t("admin.orders.editedTotal", { from: formatPrice(d.total[0]), to: formatPrice(d.total[1]) })] : []),
    ].join(" · ");
  ...
  return (
    <>
      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-4 px-4 py-5 md:px-6 print:hidden">
        <Link href="/admin/orders" className="inline-flex min-h-11 items-center self-start text-primary">
          ← {t("admin.orders.back")}
        </Link>
        ...
        <section className={box} aria-labelledby="customer">
          ...
          <p>
            <span className="font-semibold">{fulfillmentLabel(order)}</span>
          ...
          {next.length > 0 && (
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-sm text-text-muted">
                {t("admin.orders.callResult")}
                {calls > 0 && <span className="text-warning"> {t("admin.orders.calls", { n: calls })}</span>}
              </span>
              {CALL_OUTCOMES.map((o) => (
                <ActionForm key={o} action={logCall} okLabel="">
                  <input type="hidden" name="id" value={order.id} />
                  <input type="hidden" name="outcome" value={o} />
                  <SubmitButton variant="secondary">{t(`admin.orders.callOutcome.${o}`)}</SubmitButton>
                </ActionForm>
              ))}
            </div>
          )}
        ...
        <section className={box} aria-labelledby="timeline">
              ...
              return (
                <li key={h.id}>
                  ...
                  {h.action === "status" && d.from && d.to
                    ...
                    : h.action === "call" && d.outcome
                      ? t("admin.orders.callLogged", { outcome: t(`admin.orders.callOutcome.${d.outcome}`) })
                      : h.action === "edit"
                        ? t("admin.orders.edited")
                  ...
                  {h.action === "edit" && <span className="block text-text-muted">{editSummary(d)}</span>}
                </li>
              );
            })}
          </ol>
